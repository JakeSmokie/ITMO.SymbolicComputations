...
namespace ITMO.SymbolicComputations.Base.Visitors.Implementations.PowerFunction {
    public sealed class ConstantsPowerEvaluator : ISymbolVisitor<Symbol> {
        public Symbol VisitFunction(Expression power) {
            ...
            if (Equals(power.Arguments[1], new Constant(1))) return power.Arguments[0];

            if (Equals(power.Arguments[1], new Constant(0))) return 1;

            if (!(power.Arguments[0] is Constant x) || !(power.Arguments[1] is Constant scale)) return power;
